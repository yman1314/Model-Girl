...
class Com extends React.Component{
  ...
  componentDidMount () {
    api.requestReference().then(data => {
  ...
  render () {
    if(this.state.referList.length > 0){
      console.log(this.state.referList)
    }
    return (
      <div className='out'>
        <ul className='findReCen'>
          {
            this.state.referList.map((item, index) => {
              return (
                <li className='li' key={index}>
                  <NavLink to={'/home/find/reference/news/' + item._id}  className='view'>
